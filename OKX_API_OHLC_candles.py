...
start_time = int((current_time - timedelta(days=2470)).timestamp() * 1000) #smaller number goes in (days=???). Oldest is somewhere between 2400 and 2500. We start from 2500 (2018-03-22)
end_time = int((current_time - timedelta(days=2474)).timestamp() * 1000) #larger. 1h candles go for 4 days 3 hrs, so pull 4 days at a time
